      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="relative border-l-2 border-primary/40 pl-6 space-y-10">
          {experiences.map((exp, index) => (
            <div
              key={index}
              className="relative pixel-border bg-card/50 backdrop-blur-sm p-6 
                         hover:translate-x-2 transition-transform duration-300"
            >
              {/* Timeline Dot */}
              <div className="absolute -left-[14px] top-6 w-3 h-3 rounded-full bg-primary shadow-lg"></div>
              ...
              {/* Sub Info */}
              <p className="text-xs text-foreground/80 italic">{exp.location}</p>
